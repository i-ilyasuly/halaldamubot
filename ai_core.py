...
def extract_search_term(text):
    ...
    prompt = f"""Сен тек аудармашысың. Міндетің — қазақ/орыс тілінде жазылған өнім немесе мекеме атауын оның НАҚТЫ БРЕНДТІК АТАУЫНА аудару.

Ереже:
1. Тек бренд/өнім/мекеме атауын қайтар — басқа ештеңе жазба
2. Егер атау кириллицамен жазылған латын бренд болса — латынға аудар
3. Егер атау жалпы сөз болса (мысалы "шұжық", "нан", "дәмхана") — "GENERAL" деп қайтар
4. Жауабыңда тек атау болсын, түсіндірме жазба

Мысалдар:
"мексиси" → "Mexxi"
"снікерс" → "Snickers"
"марс" → "Mars"
"алматы дәмханасы" → "Алматы дәмханасы"
"шұжық" → "GENERAL"
"халал нан" → "GENERAL"
"KFC" → "KFC"
...
Жауап:"""
    ...
    try:
        ...
        if result == "GENERAL" or not result or len(result) > 50:
            return None
        # Егер бастапқы мәтінмен бірдей болса — өзгеріс жоқ
        if result.lower() == text.lower().strip():
            return None
        return result
    except Exception as e:
