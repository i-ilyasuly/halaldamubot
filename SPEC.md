...
3. Нәтижеге қарай:
   ...
   - Табылмаса → **AI#1:** `extract_search_term()` нормализациялап қайта іздейді
   - Екінші іздеу де табылмаса → **AI#2:** `get_not_found_reply()` "базада жоқ" деп айтады
...
| 2026-03-20 | AI#1 (extract_search_term) + AI#2 (get_not_found_reply) архитектурасы қосылды | `ai_core.py`, `handlers_message.py` |
| 2026-03-20 | handle_photo-ға AI#1+AI#2 логикасы қосылды: сурет табылмаса да нормализациялап іздейді | `ai_core.py` |
| 2026-03-20 | 🔄 ШЕШІЛМЕГЕН: "Мексиси"→"Mexxi" транслитерация мәселесі | `ai_core.py` |
